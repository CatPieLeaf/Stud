...
// StartAppWithParams crash (a null UserController singleton,
...
// symbols) + the app's own code of `NativeGLInterface`/
// `StartAppParams`/`StartGameParams`
...
//   - AppShellFragment.onCreateView() builds the real
//     SurfaceView; if a real "surface ready" check
//     is already true, calls its start step ->
//     the app shell's start step (-> nativeAppBridgeV2StartAppWithParams).
//   - the start step itself is gated behind
...
// flow. Its real updateSurface() is called on every real
...
// its setup step, called by AppShellFragment.onCreate() BEFORE the init step
...
// a read through
// a lazily-populated subsystem
// pointer still null. `nativeGameGlobalInit()`/`nativeUpdateAdapterInit()`
// are the two real calls AppShell's setup step makes that Stud never called at
// all, run before Init now, matching the real setup -> init -> start order.

namespace stud::jni_bridge {

struct EngineV2BridgeResult {
    // Real AppShell setup step's own two calls (nativeGameGlobalInit,
...
// a MessageBus subscription on `JNIExperienceProtocol.getLaunchId()`, and
