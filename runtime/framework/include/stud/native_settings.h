...
// (the app's own HTTP/cookie layer, class already familiar from the
...
struct NativeSettingsConfig {
    ...
    // (the app's own directory setup, NativeSettingsInterface) that this is a
    ...
    // truth-traced call site (NativeHelper: Context.
    ...
    // investigation): the app's own HTTP/cookie layer's settings bootstrap is the real, full
    // NativeSettingsInterface bootstrap sequence; Stud had only ever
    // replicated 4 of its real calls (base URL, proxy, cookies, files/
    // cache directory). Real default values below match the app's own
    // argument-computing helpers exactly, not
