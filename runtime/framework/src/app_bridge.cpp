...
// the app's own HTTP/cookie layer/k(Context)/l(Context,String) -> the app's own User-Agent builder's a() ->
// the app's own User-Agent builder's own real builder class and its c() method (~/Stud/
...
//
// Real format (the app's own User-Agent builder's c(), byte-exact):
...
// SDK_INT >= 29` (the app's own User-Agent builder's own h0()-gated i(boolean) flag),
...
std::string build_user_agent(bool android_app_token) {
    ...
    // builder (the app's own User-Agent builder -> new i().m(c()).o(j()).g(g())
    // .f(f())):
    //   1st  cm.a.j()  Display.getSize(), the display in pixels
    //   2nd  cm.a.g()  xdpi/ydpi in the real builder
    //   3rd  cm.a.f()  widthPixels/density, density-independent size
    //
    ...
    // Truncated, not rounded, matching cm.a.g()'s own `(int)` cast.
    ...
    // cm.a.c(): total system memory in MB, from ActivityManager.
    const int memory_mb = real_total_memory_mb();

    // cm.a.b(): MANUFACTURER + " " + MODEL. Deliberately NOT this
    ...
    // the app's own User-Agent builder's h(): VR, then Phone (isTablet false), then TV, else
...
AppBridgeResult run_app_bridge_start(FakeJni::Jvm& jvm, const stud::linker::LoadedLibrary& lib,
    ...
    // BuildConfig.java in the app itself), not
