...
| `STUD_SWAPCHAIN_IMAGES` | 4 | How many images to ask the real swapchain for, clamped to what the surface allows. The engine asks for 3 and only two ever reach the compositor, which leaves nothing to absorb a late buffer release. `engine` forwards the engine's own request untouched. |
