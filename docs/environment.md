...
| `STUD_PRESENT_PACING` | on | `0` stops Stud waiting for the display before letting the engine run further ahead. On, no more than two frames are in flight at once, which is what a real device's vsync-paced swap does and what bounds input lag; off, the engine runs open loop as it did before. Needs a driver with `VK_KHR_present_wait`; where there is none this does nothing either way. |
