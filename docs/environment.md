...
| `STUD_PRESENT_PACING` | off | `1` makes Stud wait for the display before letting the engine run further ahead, so no more than two frames are ever in flight. That is what a real device's vsync-paced swap does, and it bounds input lag. Off by default because it caps the frame rate at the refresh rate: the default present mode is `mailbox`, which exists to run past it. Needs a driver with `VK_KHR_present_wait`; where there is none this does nothing either way. |
