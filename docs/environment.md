...
| `STUD_UPSCALER` | `fsr` | Which spatial upscaler runs: `fsr` (FSR1, EASU then RCAS, two dispatches, twelve taps on every pixel), `sgsr` (SGSR1, one dispatch scaling and sharpening together, one bilinear fetch wherever the neighbourhood is flat), `sgsr-ed` (the same, weighted along an estimated edge direction), `ravu` (RAVU-Zoom r2: a structure tensor over the neighbourhood's luma indexes a trained table of filter weights; carries an LGPL licence, unlike the others), or `ravu-ar` (the same, clamped into the neighbourhood's own range so it cannot ring — RAVU's weights otherwise trace a border around hard edges). All are history-free, which is the only kind Stud can run: the engine hands it no motion vectors. Scaffolding for choosing between them — Stud ships one. |
