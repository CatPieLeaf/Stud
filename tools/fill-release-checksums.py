...
Flathub:
    ...
    tools/fill-release-checksums.py 1.1.3

With no network (or to redo it from values already in hand):

    tools/fill-release-checksums.py 1.1.3 --source-sha <hex> --archive-sha <hex>
...
def main() -> int:
    ...
    ap.add_argument("tag", help="the published tag, e.g. 1.1.3")
