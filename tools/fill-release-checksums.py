...
Flathub:
    ...
    tools/fill-release-checksums.py 1.1.2

With no network (or to redo it from values already in hand):

    tools/fill-release-checksums.py 1.1.2 --source-sha <hex> --archive-sha <hex>
...
def main() -> int:
    ...
    ap.add_argument("tag", help="the published tag, e.g. 1.1.2")
